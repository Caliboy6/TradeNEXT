...
const { renderGciFactory } = await import(pathToFileURL(resolve(root, 'opennext-gci.js')));
...
const factoryPath = resolve(root, 'gci-index-factory.html');
const factoryHtml = readFileSync(factoryPath, 'utf8').replace(/<!-- opennext:gci:start -->[\s\S]*?<!-- opennext:gci:end -->/, `<!-- opennext:gci:start -->${renderGciFactory()}<!-- opennext:gci:end -->`);
writeFileSync(factoryPath, factoryHtml);
