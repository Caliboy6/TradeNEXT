...
import { escapeHtml } from "./demo-core.js?v=opennext-20260913-2";
import {
  demandTape,
  quoteComparison,
  gpuSupplyListings,
...
} from "./procurement-data.js?v=opennext-20260913-2";
...
function buyerStepBody() {
  ...
  if (step === 1) return `${progress(["Quote", "Lock", "Test", "Contract", "Activate", "Receipt"], 1)}<form class="flow-form"><label><span>Allocation to lock</span><input class="input" value="${quote.capacity}"></label><label><span>Commercial total</span><input class="input" value="${quote.total}"></label><label><span>Service start</span><input class="input" type="datetime-local" value="${localDateInput(nextReservationStart())}"></label><label><span>Service term</span><input class="input" value="30 days"></label><label class="span-2 check-row"><input type="checkbox" id="commercialConfirmed" checked> I accept the quote validity, delivery, revocation and refund boundaries.</label></form><div class="workflow-callout"><span>✓</span><div><strong>Allocation hold available</strong><p>The supplier will hold the quoted capacity for 45 minutes while acceptance testing is completed.</p></div></div>`;
