...
const notice=()=>`<p class="cap-demo-note">Demo workspace · sample allocations and rates · no live infrastructure or charges${storageAvailable?'':'. Browser storage is unavailable; changes last for this visit only.'}</p>`;
...
function gpusPage() {
 ...
 <div class="cap-table-wrap" tabindex="0" aria-label="GPU reservation table"><table class="cap-table cap-gpu-table"><thead><tr><th scope="col">Reservation</th><th scope="col">Status & runtime</th><th scope="col">Time remaining</th><th scope="col">Expires</th><th scope="col">Renewal reminders</th><th scope="col"><span class="cap-visually-hidden">Actions</span></th></tr></thead><tbody>${s.gpus.map(gpuRow).join('')}</tbody></table><p class="cap-empty" id="capNoReservations"${s.gpus.some(matchesGPU)?' hidden':''}>No reservations match this search. Try a different model, region or status.</p></div><p class="cap-footnote">One GPU-hour is one GPU reserved for one hour. For example, 32 GPUs with 48 hours remaining equal 1,536 GPU-hours. Runtime is simulated elapsed reservation time, not measured GPU utilization. Reminder settings are local preferences; no notifications are sent.</p>${notice()}</section>`;
}
...
export function initializeCapacity(options={}) {
 ...
 document.addEventListener('click',event=>{
   ...
   if(actionName==='details')details(id);
     ...
     if(result.ok){close();refresh();inform('Demo extension confirmed. The reservation and sample Fees record are updated.');}
 ...
 document.addEventListener('change',event=>{
   ...
   if(toggle){const key=toggle.dataset.capacityToggle;const allowed=['usageAlert','billingReminder','renewalReminder'];if(!allowed.includes(key))return;
     ...
     item[key]=toggle.checked;save();inform(`${key==='renewalReminder'?'Renewal reminders':key==='usageAlert'?'Usage alerts':'Payment and expiry reminders'} ${toggle.checked?'enabled':'disabled'} for this demo.`);
   }
