...
const link=(label,route,primary=false)=>`<button type="button" class="cap-button${primary?' cap-button-dark':''}" ${['models','gpus'].includes(route)?'data-marketplace-action="create-rfq"':`data-route="${esc(route)}"`}>${label} ${arrow}</button>`;
...
function profilePage() {
  ...
  return `<section class="on-capacity" data-capacity-page="profile">${head('MY OPENNEXT / PROFILE','Your workspace, at a glance.','Keep an eye on capacity, usage and the next decision.',link('Post RFQ','gpus',true))}
...
function renewal(id) {
 ...
 if(calculateReservation(g).status==='Expired'){inform('This reservation has expired. Post an RFQ to source a new allocation.');return;}
 pendingRenewal=null;
