import { initializePublic, initializePublicShell, reportWorkspaceFailure } from './opennext-public.js?v=opennext-20260913-1';
import './opennext-a11y.js?v=opennext-20260913-1';
...
try {
  await import('./phase1-entry-v2.js?v=opennext-20260913-1');
  const { procurementState } = await import('./procurement-data.js?v=opennext-20260913-1');
  window.OpenNEXTSetGpuMode = mode => { procurementState.gpuMode = mode === 'hardware' ? 'hardware' : 'rental'; };
  const { initializeWorkspaceShell } = await import('./opennext-workspace-shell.js?v=opennext-20260913-1');
  initializeWorkspaceShell();
  const { initializePortal } = await import('./opennext-portal.js?v=opennext-20260913-1');
  initializePortal();
