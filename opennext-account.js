import { getCapacityLedgerRecords } from './opennext-capacity.js?v=opennext-20260913-1';
...
function transactions() {
  ...
  let balance = 10000;
  const records = [{ id: 'TXN-DEMO-1000', date: new Date(Math.min(...ordered.map((r) => Date.parse(r.date)), Date.now()) - 86400000).toISOString(), title: 'Opening demo credit', category: 'Funding', amount: 10000, direction: 'in', balance: 10000, channel: 'Demo credit', status: 'Completed' }];
  for (const item of ordered) { balance -= item.amount; records.push({ ...item, direction: 'out', balance: Math.round(balance * 100) / 100, channel: 'Demo credit' }); }
...
function renderSecurity() {
  return `<div class="ac-section-head"><div><h2>Security</h2><p>Review sign-in methods and workspace access.</p></div>${badge('Demo account')}</div><section class="ac-settings-card" aria-label="Sign-in security">${securityRow('Password', state.passwordReset ? 'A password reset has been simulated in this browser.' : 'Manage password-based access to your workspace.', badge(state.passwordReset ? 'Reset simulated' : 'Password set'), button('Reset password', 'password'))}${securityRow('Two-factor authentication', 'Add an extra verification step to password sign-in.', badge(state.mfa ? 'Enabled in demo' : 'Not enabled', state.mfa ? 'ac-badge-ok' : ''), button(state.mfa ? 'Manage' : 'Set up', 'mfa'))}${securityRow('Recovery phone', state.phone ? '+1 (202) 555-0142 · sample number' : 'An optional recovery method for your workspace.', badge(state.phone ? 'Demo linked' : 'Not linked'), button(state.phone ? 'Remove' : 'Add sample phone', 'phone'))}${securityRow('Organization verification', 'Example Organization · business procurement account.', badge('Sample verified'), button('View details', 'verification'))}</section><div class="ac-section-head ac-subsection"><div><h2>Connected accounts</h2><p>Choose how you sign in to OpenNEXT.</p></div></div><section class="ac-settings-card" aria-label="Connected accounts">${['Google','GitHub','Lark','WalletConnect','Binance','MetaMask'].map((p) => securityRow(p, /WalletConnect|Binance|MetaMask/.test(p) ? 'Wallet sign-in preview' : 'Single sign-on preview', badge(state.providers.includes(p) ? 'Demo connected' : 'Not connected'), button(state.providers.includes(p) ? 'Disconnect' : 'Connect', 'provider', `data-provider="${p}"`))).join('')}</section><div class="ac-section-head ac-subsection"><div><h2>Active sessions</h2><p>Sample devices with access to this workspace.</p></div>${button('Revoke other sessions', 'revoke-all', `${sessions.filter((s) => !s.current && !state.revokedSessions.includes(s.id)).length ? '' : 'disabled'}`)}</div>${table(['Device', 'Location / IP', 'Last active', 'Status', ''], sessions.map((s) => `<tr><td>${s.device}</td><td>${s.region}<small class="ac-mono">${s.ip}</small></td><td>${dateLabel(s.date)}</td><td>${badge(s.current ? 'This session' : state.revokedSessions.includes(s.id) ? 'Revoked' : 'Active')}</td><td>${s.current ? '<span class="ac-muted">Current</span>' : button('Revoke', 'revoke', `data-id="${s.id}" ${state.revokedSessions.includes(s.id) ? 'disabled' : ''}`, 'ac-button-quiet')}</td></tr>`).join(''), 'Sample sign-in sessions')}<p class="ac-note">Security controls demonstrate the intended workflow. They do not secure this public demo or connect real accounts.</p><div class="ac-signout"><div><h3>Current demo session</h3><p>End this session and return to sign in.</p></div><button type="button" class="ac-button" data-flow-action="logout">Sign out</button></div>`;
}
...
function renderSettings() {
  ...
  return `<div class="ac-section-head"><div><h2>Workspace settings</h2><p>Manage your profile, preferences and developer access.</p></div></div><form data-ac-form="settings" class="ac-settings-form"><section class="ac-settings-card"><div class="ac-card-heading"><h3>Profile &amp; preferences</h3><p>A generic workspace identity for your demo.</p></div><div class="ac-form-grid"><label>Display name<input name="name" maxlength="60" value="${escape(s.name)}" required></label><label>Organization<input name="organization" maxlength="100" value="${escape(s.organization)}" required></label><label>Contact email<input name="email" type="email" maxlength="120" value="${escape(s.email)}" required></label><label>Time zone<select name="timezone">${['UTC','America/Los_Angeles','America/New_York','Europe/London','Asia/Singapore','Asia/Tokyo'].map((z) => `<option${s.timezone === z ? ' selected' : ''}>${z}</option>`).join('')}</select></label><label>Language<input value="English" disabled aria-describedby="ac-language-note"><small id="ac-language-note">English is the workspace language.</small></label><div class="ac-static-field"><span>Currency</span><strong>USD · US dollar</strong><small>All sample quotes and statements use USD.</small></div></div></section><section class="ac-settings-card"><div class="ac-card-heading"><h3>Notifications</h3><p>Preferences are saved locally; no messages are sent.</p></div>${switchField('weekly','Weekly activity summary','A recap of capacity, purchases and RFQs.',s.weekly)}${switchField('billing','Billing updates','Invoices, statements and payment reminders.',s.billing)}${switchField('renewal','Renewal reminders','Upcoming capacity expirations and renewals.',s.renewal)}${switchField('security','Security alerts','Sign-in and account-change notifications.',s.security)}</section><div class="ac-form-actions"><span data-ac-save-note>${persistent ? 'Changes apply to this demo browser.' : 'Browser storage is unavailable. Changes last for this tab.'}</span><button class="ac-button ac-button-dark" type="submit">Save preferences</button></div></form><div class="ac-section-head ac-subsection"><div><h2>API access</h2><p>Preview scoped credentials for procurement integrations.</p></div>${button('Create demo key', 'api-add')}</div>${state.apiKeys.length ? table(['Name / created', 'Scope', 'Key reference', 'Status', ''], state.apiKeys.map((k) => `<tr><td>${escape(k.name)}<small>${dateLabel(k.date)}</small></td><td>${escape(k.scope)}</td><td class="ac-mono">${escape(k.reference)}</td><td>${badge(k.revoked ? 'Revoked' : 'Demo only')}</td><td>${button('Revoke', 'api-revoke', `data-id="${escape(k.id)}" ${k.revoked ? 'disabled' : ''}`, 'ac-button-quiet')}</td></tr>`).join(''), 'Demo API key references') : '<div class="ac-key-empty">No API key previews yet. Demo keys cannot authenticate requests.</div>'}<div class="ac-section-head ac-subsection"><div><h2>SSH public keys</h2><p>Review the keys you would use to access provisioned compute.</p></div>${button('Add public key', 'ssh-add')}</div>${state.sshKeys.length ? table(['Name / added', 'Key type', 'Fingerprint preview', ''], state.sshKeys.map((k) => `<tr><td>${escape(k.name)}<small>${dateLabel(k.date)}</small></td><td>${escape(k.type)}</td><td class="ac-mono">${escape(k.fingerprint)}</td><td>${button('Remove', 'ssh-remove', `data-id="${escape(k.id)}"`, 'ac-button-quiet')}</td></tr>`).join(''), 'SSH public key previews') : '<div class="ac-key-empty">No public key previews added. Private keys are never requested.</div>'}<p class="ac-note">API and SSH controls are previews. No credentials are provisioned, and no server access is granted.</p>`;
}
...
function showDialog(title, content, footer = '') {
  ...
  dialog.innerHTML = `<header><div><span class="ac-eyebrow">OpenNEXT · demo</span><h2 id="ac-dialog-title">${title}</h2></div>${button('×', 'close-dialog', 'aria-label="Close dialog"', 'ac-dialog-close')}</header><div class="ac-dialog-body">${content}</div>${footer ? `<footer>${footer}</footer>` : ''}`;
...
async function submitForm(form) {
  ...
  if (kind === 'invoice') {
    ...
    state.invoices.unshift(record); persist(); closeAccountDialogs(); view.billing='invoices'; view.page=1; view.query=''; view.from=''; view.to=''; refresh('Invoice preview created. No email was sent.'); invoiceDialog(record.id); return;
  }
  ...
  if (kind === 'settings') {
    ...
    confirmSaved('Preferences saved. No notifications are sent in this demo.'); return;
  }
  if (kind === 'api') {
