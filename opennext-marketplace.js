...
function rentalAgreementFields(kind, values) {
  ...
  return `<section class="mp-agreement-flow" aria-label="Standard GPU rental agreement"><div class="mp-agreement-flow-head"><div><p class="mp-step-label">STANDARD GPU RENTAL AGREEMENT · V1</p><strong>${seller ? 'Seller-side terms that can be reused' : 'Buyer-side fast match first'}</strong><p>${flowCopy}</p></div><span class="mp-agreement-badge">${seller ? 'SELLER PREFILL' : 'BUYER INTAKE'}</span></div>${stages}${fields}</section>`;
}
