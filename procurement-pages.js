...
export function renderModels() {
  ...
  return `<div class="page procurement-market">
    ...
    ${referenceGuide()}
    <section class="section demand-dashboard"><div class="section-header"><div><div class="eyebrow">LIVE DEMAND TAPE</div><h2>Structured buyer demand</h2><p>Demand is anonymized and available to qualified suppliers under the selected scope.</p></div><button class="secondary-button compact" type="button" data-route="supply">Supply / Respond to RFQs</button></div><div class="demand-tape procurement-tape"><div class="demand-grid-head"><span>Requirement</span><span>Notional / term</span><span>TPS / region</span><span>Provenance / responses</span><span>Action</span></div>${demandTape.map((item) => demandRow(item, true)).join("")}</div></section>
  </div>`;
}
...
export function renderSupply() {
  ...
  return `<div class="page supply-workspace"><div class="page-head"><div><div class="eyebrow">SUPPLY-SIDE WORKSPACE</div><h1>Supply / Respond to RFQs</h1><p>Onboard model capacity, accelerator rentals or physical GPU inventory, then respond to structured buyer demand.</p></div><div class="button-row"><button class="secondary-button" type="button" data-flow-action="supplier-onboarding-full">${approved ? "View Supplier Review" : "Complete Supplier Review"}</button><button class="primary-button" type="button" data-flow-action="add-supply-picker">+ Add Supply</button></div></div>
...
export function renderGpus() {
  ...
  if (hardwareMode) {
    ...
    return `<div class="page gpu-procurement"><div class="page-head"><div><div class="eyebrow">GPU PROCUREMENT</div><h1>GPU Market</h1><p>Compare cloud capacity reservations separately from whole-GPU hardware purchase opportunities.</p></div><button class="primary-button compact" type="button" data-flow-action="add-supply" data-supply-type="hardware">+ Sell GPU inventory</button></div>
