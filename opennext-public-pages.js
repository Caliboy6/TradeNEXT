...
function publicAgentDemo() {
  const steps = [
    ['Natural-language request', '64 H100 · 1 month · $210k/month', 'The buyer starts with a plain-language request. No workflow setup is required.'],
    ['Structure the RFQ', 'Standard GPU Rental RFQ', 'The Agent turns the request into a standard RFQ suppliers can quote against.'],
    ['Complete match fields', 'Region · start · SLA · network', 'The RFQ adds only the required matching fields missing from the original request.'],
    ['Run the VETA guardrail', 'Privacy · authority · policy', 'Before outreach, VETA checks privacy, authority and the buyer’s stated parameters.'],
    ['Authorize matching', '$500 fee · optional early reserve', 'The buyer approves supplier matching and pays the $500 sourcing fee. An optional reserve can be pre-funded to lock capacity faster.'],
    ['Search seller supply', '20 private supply lists', 'The Agent compares the RFQ with seller supply lists while keeping identities and transaction details private.'],
    ['Present three matches', '3 candidates · 1 best fit', 'The Agent returns three outcomes with price, timing and fit signals. One is highlighted as the strongest match.'],
    ['Select the best fit', 'Supplier 03 · buyer selected', 'The buyer selects the best-fit supply. The seller is then asked to complete the shared record.'],
    ['Push to the seller', 'Private seller push sent', 'A private push sends the buyer-approved RFQ and selected match context to the seller.'],
    ['Seller completes terms', 'Seller terms completed', 'The seller completes reusable supply terms and the remaining delivery, SLA and acceptance fields.'],
    ['Buyer confirms terms', 'Buyer review · agreement v1', 'The Agent sends the shared English rental agreement back to the buyer for final confirmation.'],
    ['Complete the order', 'Final payment · capacity reserved', 'After buyer confirmation and final payment, the order is recorded and capacity is reserved.'],
  ];
  ...
  const factSets = [
    [['Input', 'Plain language'], ['Intent', 'GPU rental'], ['State', 'Captured']],
    [['Document', 'Standard RFQ'], ['Scope', 'Buyer-approved'], ['State', 'Draft']],
    [['Region', 'Singapore'], ['Start', 'As soon as available'], ['Requirements', '99.95% SLA · no extra']],
    [['Privacy', 'Masked'], ['Authority', 'Approval gated'], ['Result', 'Guarded']],
    [['Sourcing fee', '$500'], ['Early reserve', 'Optional · $105,000'], ['Order', 'Not placed']],
    [['Input', 'Seller supply lists'], ['Reviewed', '20'], ['Visibility', 'Private']],
    [['Candidates', '3 outcomes'], ['Best fit', 'Supplier 03'], ['Signal', '96% fit']],
    [['Decision', 'Supplier 03'], ['Counterparty', 'Masked'], ['Next', 'Seller response']],
    [['Push', 'RFQ + match context'], ['Recipient', 'Supplier 03'], ['Status', 'Sent']],
    [['Seller fields', 'Terms + evidence'], ['Shared doc', 'Rental Agreement v1'], ['Status', 'Complete']],
    [['Buyer action', 'Confirm terms'], ['Seller fields', 'Complete'], ['Status', 'Pending']],
    [['Payment', 'Final approved'], ['Reference', 'ON-DEMO-2409'], ['Status', 'Reserved']],
  ];
  ...
  const detailMarkup = steps.map(([title, summary, body], index) => '<section class="public-demo-step-detail" id="public-agent-step-detail-' + index + '" data-agent-detail="' + index + '"' + (index ? ' hidden' : '') + '><span class="agent-kicker">Step ' + String(index + 1).padStart(2, '0') + ' / ' + title + '</span><h4>' + title + '</h4><p>' + body + '</p><div class="public-agent-demo-facts">' + factSets[index].map(([label, value]) => '<span><small>' + label + '</small><strong>' + value + '</strong></span>').join('') + '</div></section>').join('');
  const stageMarkup = [
    '<div class="public-agent-stage-kicker">NATURAL LANGUAGE / CAPTURED</div><h4>Buyer request captured</h4><p>“64 H100 GPUs for one month, with a monthly budget of $210,000.”</p><div class="public-agent-stage-line"><span>Input<strong>Plain language</strong></span><span>Output<strong>Private brief</strong></span></div>',
    '<div class="public-agent-stage-kicker">STANDARD RFQ / DRAFT</div><h4>Standard RFQ prepared</h4><div class="public-agent-stage-grid"><span><small>Hardware</small><strong>64 × H100</strong></span><span><small>Term</small><strong>1 month</strong></span><span><small>Budget</small><strong>$210,000 / month</strong></span><span><small>Scope</small><strong>Buyer-approved</strong></span></div>',
    '<div class="public-agent-stage-kicker">MATCH FIELDS / COMPLETED</div><h4>Required fields added</h4><div class="public-agent-stage-grid"><span><small>Region</small><strong>Singapore</strong></span><span><small>Start</small><strong>As soon as available</strong></span><span><small>SLA</small><strong>99.95% target</strong></span><span><small>Network</small><strong>No extra preference</strong></span></div>',
    '<div class="public-agent-stage-kicker">VETA / GUARDED</div><h4>Guardrail passed</h4><ul class="public-agent-stage-checks"><li>Private identities excluded</li><li>Supplier outreach stays approval-gated</li><li>Request remains within stated parameters</li></ul>',
    '<div class="public-agent-stage-kicker">AUTHORIZATION / REQUIRED</div><h4>Matching authorization confirmed</h4><div class="public-agent-stage-grid"><span><small>Sourcing fee</small><strong>$500</strong></span><span><small>Order credit</small><strong>−$500</strong></span><span><small>Early reserve</small><strong>Optional · $105,000</strong></span><span><small>Authority</small><strong>Buyer approval</strong></span></div><p class="public-agent-stage-note">The reserve is separate from the sourcing fee and does not place an order.</p>',
    '<div class="public-agent-stage-kicker">SUPPLY SEARCH / PRIVATE</div><h4>Seller supply screened</h4><div class="public-agent-stage-line"><span>Seller lists<strong>20 reviewed</strong></span><span>Match keys<strong>Model · region · term · SLA</strong></span></div><p class="public-agent-stage-note">Identities and transaction details remain private.</p>',
    '<div class="public-agent-stage-kicker">MATCH RESULTS / 3 OUTCOMES</div><h4>Three candidates returned</h4><div class="public-agent-match-list"><div class="public-agent-match-row"><span>01</span><strong>Supplier 01</strong><em>Over budget</em></div><div class="public-agent-match-row"><span>02</span><strong>Supplier 02</strong><em>Start mismatch</em></div><div class="public-agent-match-row is-best"><span>03</span><strong>Supplier 03</strong><em>96% best fit</em></div></div>',
    '<div class="public-agent-stage-kicker">BUYER DECISION / RECORDED</div><h4>Best-fit supply selected</h4><div class="public-agent-selection"><span>Selected match</span><strong>Supplier 03 · masked</strong><em>96% fit</em></div><p class="public-agent-stage-note">The seller response is the next gated action.</p>',
    '<div class="public-agent-stage-kicker">SELLER PUSH / SENT</div><h4>Seller notified privately</h4><div class="public-agent-notification"><span>Private push</span><strong>RFQ + selected match sent</strong><small>Supplier 03 · buyer-approved fields · identity protected</small></div>',
    '<div class="public-agent-stage-kicker">SHARED AGREEMENT / SELLER SIDE</div><h4>Seller completes the rental record</h4><div class="public-agent-stage-grid"><span><small>Reusable terms</small><strong>Billing · support · remedy</strong></span><span><small>Shared terms</small><strong>Delivery · SLA · acceptance</strong></span><span><small>Evidence</small><strong>Capacity + availability</strong></span><span><small>Status</small><strong>Seller complete</strong></span></div>',
    '<div class="public-agent-stage-kicker">BUYER CONFIRMATION / PENDING</div><h4>Agreement returned for review</h4><div class="public-agent-stage-document"><span>OpenNEXT GPU Rental Agreement v1</span><strong>Seller terms complete · buyer confirmation next</strong></div><p class="public-agent-stage-note">Agent or Message can resolve any remaining shared term.</p>',
    '<div class="public-agent-stage-kicker">ORDER / RESERVED</div><h4>Order completed</h4><div class="public-agent-stage-success"><span>✓</span><div><strong>ON-DEMO-2409</strong><small>64 × H100 · Singapore · 30 days</small></div><em>Capacity reserved</em></div>',
  ].map((content, index) => '<section class="public-agent-stage" data-agent-stage="' + index + '"' + (index ? ' hidden' : '') + '>' + content + '</section>').join('');
  ...
  return '<article class="public-agent-demo" data-agent-demo><header class="public-agent-demo-head"><div><p class="public-demo-eyebrow">PROCUREMENT / AGENT DEMO</p><h3>See the Agent work, one step at a time.</h3><p>Choose a step to see what the workspace Agent is doing. This read-only preview waits for your click.</p></div><div class="public-agent-demo-status"><span data-agent-demo-dot></span><strong data-agent-demo-status>Ready</strong><small data-agent-demo-progress>Step 01 / 12</small></div></header><div class="public-agent-demo-shell"><section class="public-demo-explainer" aria-label="Procurement Agent steps"><div class="public-demo-explainer-intro"><p class="public-demo-eyebrow">SELECT A STEP</p><h4>From brief to order.</h4><p>Click a stage below. The short note explains the action; the Agent preview updates on the right.</p></div><nav class="public-demo-step-list" aria-label="Procurement Agent steps">' + stepMarkup + '</nav><div class="public-demo-step-details">' + detailMarkup + '</div></section><aside class="public-demo-agent-panel agent-panel" aria-label="OpenNEXT Agent step preview"><div class="agent-heading"><div class="agent-identity"><span class="agent-symbol">' + sparkle + '</span><div><h2>OpenNEXT Agent</h2><p>From request to reserved capacity</p></div></div><span class="public-demo-auto-badge">Read-only flow</span></div><div class="agent-context"><span>Active brief</span><strong>64 × H100 · Singapore · 30 days</strong></div><div class="agent-scroll"><div class="agent-intro"><p>This mirrors the Agent view inside My OpenDesk. Select a step on the left to inspect the workflow.</p></div><div class="agent-conversation" role="log" aria-label="Agent workflow preview" aria-live="polite"><article class="agent-message agent-message-user"><span class="agent-speaker">Buyer brief</span><p>I’d like 64 H100 GPUs for one month, with a monthly budget of $210,000. Can you find suitable suppliers?</p></article><article class="agent-message"><span class="agent-speaker">OpenNEXT Agent</span><p data-agent-demo-message>I’ll turn this plain-language request into a private RFQ.</p></article><section class="agent-card public-agent-demo-active-card"><div class="public-agent-demo-activity"><span class="agent-kicker">Execution activity · selected step</span><strong data-agent-demo-activity>Capturing the natural-language request</strong><em data-agent-demo-stage-tag>Selected</em></div><div class="public-agent-demo-agent-state"><span>Current state</span><strong data-agent-demo-agent-status>Natural-language request</strong><small data-agent-demo-progress>Step 01 / 12</small></div><div class="public-agent-demo-stage-stack">' + stageMarkup + '</div></section></div></div><div class="public-demo-agent-footer"><span>Approval before action</span><strong>Read-only presentation</strong></div></aside></div><footer class="public-agent-demo-footer-bar"><span>Click a step to update the Agent preview</span><span>Read-only simulation · no payment or allocation occurs</span></footer></article>';
}
