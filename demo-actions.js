...
import { renderOverview, renderModels, renderGpus, renderScheduler, renderRfq, renderData } from "./demo-pages.js?v=opennext-20260914-desk-1";
...
function showModal(title, caption, body, footer = "") {
  modal.innerHTML = `<div class="modal-backdrop" data-final-action="close-modal"><section class="modal" role="dialog" aria-modal="true" aria-label="${escapeHtml(title)}" data-modal-panel><header class="modal-head"><div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(caption)}</p></div><button class="close-button" type="button" data-final-action="close-modal" aria-label="Close">×</button></header><div class="modal-body">${body}</div>${footer ? `<footer class="modal-footer">${footer}</footer>` : ""}</section></div>`;
...
function showBuy(modelId, providerId) {
  ...
  showModal(`Buy ${model.name} Capacity`, "INSTANT BUY · LANE A · SIMULATION", `<div class="scheduler-note"><span>ⓘ</span><div><strong>Controlled capacity delivery</strong> The buyer accesses capacity through OpenNEXT Gateway and never receives the supplier's raw key.</div></div><div class="form-grid" style="margin-top:14px"><div class="form-field"><label>Official-equivalent usage</label><input class="input" value="$10,000 OEV"></div><div class="form-field"><label>Supply route</label><select class="select">${depth.map((item) => `<option ${item.id === provider?.id ? "selected" : ""}>${escapeHtml(item.providerName)} · ${Number(item.priceMultiple).toFixed(2)}×</option>`).join("")}</select></div></div><div class="quote-summary"><div class="quote-line"><span>Capacity cost</span><strong>$${subtotal.toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong></div><div class="quote-line"><span>Platform service fee (1.5%)</span><strong>$${fee.toFixed(2)}</strong></div><div class="quote-line total"><span>Estimated total</span><strong>$${(subtotal + fee).toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong></div></div>`, `<button class="secondary-button compact" type="button" data-final-action="close-modal">Cancel</button><button class="primary-button compact" type="button" data-final-action="confirm-order">Reserve & Continue</button>`);
}

function showGpu(id) {
  const raw = gpuOffers.find((item) => item.id === id) || gpuOffers[0];
  const gpu = gpuView(raw);
  showModal(`${gpu.name} · ${gpu.region}`, "GPU CAPACITY · VERIFIED CALENDAR · DEMO", `<div class="summary-grid"><div class="summary-card is-highlight"><div class="summary-label">Price</div><div class="summary-value">$${gpu.unitPrice.toFixed(2)}</div><div class="summary-context">per card-hour</div></div><div class="summary-card"><div class="summary-label">Available</div><div class="summary-value">${gpu.available}</div><div class="summary-context">cards</div></div><div class="summary-card"><div class="summary-label">Start</div><div class="summary-value" style="font-size:15px">${escapeHtml(String(gpu.start))}</div><div class="summary-context">verified calendar</div></div><div class="summary-card"><div class="summary-label">SLA</div><div class="summary-value">${gpu.sla.toFixed(2)}%</div><div class="summary-context">${escapeHtml(gpu.term)}</div></div></div><div class="scheduler-note" style="margin-top:14px"><span>G</span><div><strong>${escapeHtml(gpu.vendor)}</strong> · ${escapeHtml(gpu.topology)}<br>Standard accelerator-hours can be reserved directly; large clusters, long terms, private networks and custom images enter Private RFQ.</div></div>`, `<button class="secondary-button compact" type="button" data-final-action="new-rfq" data-family="${escapeHtml(gpu.name)}">Create RFQ</button><button class="primary-button compact" type="button" data-final-action="confirm-gpu">Reserve demo capacity</button>`);
}

function showRfq(family = "Claude") {
  showModal("Create a Private RFQ", "BROKERED OTC · INVITE ONLY", `<div class="scheduler-note"><span>R</span><div><strong>Separate from the public market</strong> The RFQ is not public, does not count as executable inventory and is excluded from the core index.</div></div><div class="form-grid" style="margin-top:14px"><div class="form-field"><label>Requirement type</label><select class="select"><option>${escapeHtml(family)} capacity</option><option>Custom AI capacity</option></select></div><div class="form-field"><label>Budget / OEV</label><input class="input" value="$100,000"></div><div class="form-field"><label>Term</label><select class="select"><option>30 days</option><option>90 days</option></select></div><div class="form-field"><label>Minimum throughput</label><input class="input" value="2M TPM / 1,200 RPM"></div><div class="form-field"><label>Data region</label><select class="select"><option>US</option><option>Singapore</option><option>Europe</option></select></div><div class="form-field"><label>Controlled delivery</label><select class="select"><option>OpenNEXT managed gateway</option><option>Dedicated endpoint</option></select></div></div>`, `<button class="secondary-button compact" type="button" data-final-action="close-modal">Cancel</button><button class="primary-button compact" type="button" data-final-action="submit-rfq">Invite verified suppliers</button>`);
}
...
function showRfqResponses() {
  ...
  showModal(rfq.title, "PRIVATE QUOTE ROOM · SYNTHETIC RESPONSES", `<div class="table-wrap"><table class="data-table"><thead><tr><th>Responder</th><th>Indicative</th><th>Verified throughput</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div><div class="scheduler-note" style="margin-top:14px"><span>ⓘ</span><div>Quote, test and transaction data may feed a separate OTC indicator but never Lane A depth or the core TMCI.</div></div>`);
}

function showGuide() {
  showModal("4-minute demo flow", "PRESENTER MODE", `<div class="lane-stack"><div class="lane-row"><span class="badge badge-blue">01</span><div><strong>Two markets</strong><span>Model Capacity is inference retail; GPU Capacity is compute wholesale.</span></div></div><div class="lane-row"><span class="badge badge-blue">02</span><div><strong>Supply depth</strong><span>Open Claude to view Lane A instant execution and the separate Private RFQ.</span></div></div><div class="lane-row"><span class="badge badge-blue">03</span><div><strong>Smart orchestration</strong><span>After task decomposition, select the model, supply route and disclosed chip for each step.</span></div></div><div class="lane-row"><span class="badge badge-blue">04</span><div><strong>Failover receipt</strong><span>Simulate degraded health, automatic failover and Plan vs Actual auditing.</span></div></div></div>`, `<button class="primary-button compact" type="button" data-final-action="start-guide">Start demo</button>`);
}
...
async function runSimulation() {
  ...
  for (let index = 0; index < plan.steps.length; index += 1) {
    ...
    if (!fallbackEvent && index >= Math.floor(plan.steps.length / 2)) {
      try {
        ...
        renderPage("scheduler");
        showToast("Primary route health declined; switched automatically to fallback", `${result.event.detectedInMs + result.event.reroutedInMs} ms recovery · audit logged`);
        await wait(420);
        if (leftScheduler()) return;
      } catch (error) {
  ...
  state.scheduler.plan = plan;
  renderPage("scheduler");
  showToast("Execution complete", "Results, per-step usage, failover and settlement receipt are ready");
...
function handleAction(action, element) {
  ...
  if (action === "confirm-order") { closeOverlays(); return showToast("Demo order created", "Capacity reserved and deducted from executable inventory in real time"); }
  if (action === "confirm-gpu") { closeOverlays(); return showToast("GPU capacity reserved", "Demo calendar updated"); }
  if (action === "submit-rfq") { closeOverlays(); return showToast("RFQ sent to 6 qualified suppliers", "The broker will coordinate capacity tests and private quotes"); }
  ...
  if (action === "reserve-plan") return showToast("All execution targets reserved", "Lane A capacity reservation · Demo Simulation");
  if (action === "export-trace") return showToast("Redacted Trace ready to export", "Policy, metering, errors and failover records are included");
...
document.addEventListener("change", (event) => {
  ...
  if (target.matches?.("[data-action='override-route']")) {
    ...
    try {
      ...
      state.scheduler.completed = false;
      renderPage("scheduler");
      showToast("Manual route applied", "Critical path, cost and quality recalculated");
    } catch (error) {
      showToast("Route does not satisfy current hard constraints", error.message);
    }
  }
