...
function input(name, values, options = {}) {
  ...
  return `<input id="mp-${name}" name="${name}" type="${type}" value="${escape(values[name])}" ${required ? 'required' : ''}${min !== undefined ? ` min="${min}"` : ''}${max !== undefined ? ` max="${max}"` : ''}${step !== undefined ? ` step="${step}"` : ''}${list ? ` list="${list}"` : ''} ${type === 'text' ? 'maxlength="100"' : ''} placeholder="${escape(placeholder)}" autocomplete="off">`;
}
function select(name, values, options) {
  return `<select id="mp-${name}" name="${name}" required>${options.map(([value, label]) => `<option value="${escape(value)}" ${String(values[name]) === value ? 'selected' : ''}>${label}</option>`).join('')}</select>`;
}
...
function defaultValues(kind, market) {
  if (market === 'gpu') return kind === 'supply' ? { model: '', servers: '', gpusPerServer: 8, region: 'United States', minimumMonths: 1, monthlyPrice: '', slaPercent: 99.95, availability: 'available', availableFrom: today(), allowPartial: true } : { model: '', gpuCount: '', region: 'Any region', durationMonths: 1, monthlyBudget: '', minSlaPercent: 99.9, startDate: today() };
...
function detailsRows(record, kind, includeNotes = true) {
  ...
  if (record.market === 'gpu' && kind === 'supply') rows.push(['Servers / GPUs per server', `${number(record.servers)} / ${number(record.gpusPerServer)}`], ['Total capacity', `${number(record.totalGpus)} GPUs`], ['Operating region', record.region], ['Monthly asking price', `${money(record.monthlyPrice)} for the full inventory`], ['Partial allocations', record.allowPartial ? 'Available at a proportional monthly price' : 'Full inventory only'], ['Minimum lease', `${number(record.minimumMonths)} months`], ['Uptime SLA', `${number(record.slaPercent)}%`], ['Availability', record.availability === 'unavailable' ? 'Currently unavailable' : `Available ${date(record.availableFrom)}`]);
  else if (record.market === 'gpu') rows.push(['GPUs required', number(record.gpuCount)], ['Required region', record.region], ['Monthly budget', money(record.monthlyBudget)], ['Lease duration', `${number(record.durationMonths)} months`], ['Requested start', date(record.startDate)], ['Minimum uptime SLA', `${number(record.minSlaPercent)}%`]);
