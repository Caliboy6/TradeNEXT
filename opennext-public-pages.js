import { renderGciFactory } from './opennext-gci.js?v=opennext-20260915-gci-1';
...
export function renderLanding(locale = 'en') {
  ...
  return `<div class="public-page" lang="${locale}">
    <header class="public-header"><div class="public-header-inner">${brandLink()}<nav class="public-nav" aria-label="${c.market}"><button data-public-action="scroll" data-section="market">${c.market}</button><button data-public-action="scroll" data-section="how-it-works">${c.how}</button><button data-public-action="scroll" data-section="public-gci">Index Factory</button><button data-public-action="scroll" data-section="suppliers">${c.suppliers}</button></nav><div class="public-header-actions"><button class="on-button on-button-primary on-button-small" data-public-action="signin">${c.signin}</button></div></div></header>
    ${gpuTicker()}
    <main>
      ...
      <section class="public-section public-market-section public-container" id="market" aria-labelledby="public-market-title"><div class="public-section-heading"><p class="on-eyebrow">${c.marketLabel}</p><h2 id="public-market-title">${c.marketTitle}</h2></div><div class="public-market-row"><span class="public-row-number">01</span><h3>${c.models}</h3><p>${c.modelDescription}</p>${cta(c.modelPreview, 'models', 'text')}</div><div class="public-market-row"><span class="public-row-number">02</span><h3>${c.gpu}</h3><p>${c.gpuDescription}</p>${cta(c.gpuPreview, 'gpus', 'text')}</div></section>
      <section class="public-listings-band" aria-labelledby="public-listings-title"><div class="public-section public-listings-section public-container"><div class="public-section-heading"><p class="on-eyebrow">${c.inside}</p><h2 id="public-listings-title">${c.listingsTitle}</h2><p class="public-section-description">${c.listingsDescription}</p></div><div class="public-preview-label">${c.example}</div>${listings(c)}<ol class="public-posting-steps">${c.steps.map((step, i) => `<li><span>0${i + 1}</span><p>${step}</p></li>`).join('')}</ol><div class="public-preview-actions">${cta(c.viewMarket)}${cta(c.publish, 'supply', 'text')}</div></div></section>
      <section class="public-section public-procurement-section public-container" id="how-it-works" aria-labelledby="public-process-title"><div class="public-section-heading"><p class="on-eyebrow">${c.procurement}</p><h2 id="public-process-title">${c.procurementTitle}</h2><p class="public-section-description">A clear record of every decision, from the first quote to the final delivery.</p></div><ol class="public-process">${c.process.map(([title, description], i) => `<li><span class="public-process-number">0${i + 1}</span><h3>${title}</h3><p>${description}</p></li>`).join('')}</ol></section>
      <section class="public-gci-band" id="public-gci" aria-labelledby="public-gci-title"><div class="public-container public-gci-section"><div class="public-gci-heading"><div><p class="on-eyebrow">04 / THE INDEX FACTORY</p><h2 id="public-gci-title">OpenNEXT GCI Index Factory</h2><p class="public-section-description">System Architecture and End-to-End Workflow for the GPU Compute Index.</p></div><a class="on-button on-button-text" href="./gci-index-factory.html" target="_blank" rel="noopener">Open full view <span aria-hidden="true">↗</span><span class="public-sr-only">(opens in a new tab)</span></a></div>${renderGciFactory()}</div></section>
      <section class="public-supplier-band" id="suppliers" aria-labelledby="public-supplier-title"><div class="public-container public-supplier-inner"><div class="public-supplier-heading"><p class="on-eyebrow">05 / A TWO-SIDED MARKET</p><h2 id="public-supplier-title">${c.band}</h2></div><div class="public-supplier-column"><p class="on-eyebrow">FOR BUYERS</p><p>${c.buyer}</p>${cta(c.buyerAction, 'rfq', 'text')}</div><div class="public-supplier-column"><p class="on-eyebrow">FOR SUPPLIERS</p><p>${c.supplier}</p>${cta(c.supplierAction, 'supply', 'text')}<small>${c.supplierNote}</small></div></div></section>
      <section class="public-closing public-container" aria-labelledby="public-closing-title"><div><p class="on-eyebrow">06 / GET STARTED</p><h2 id="public-closing-title">${c.closing}</h2><p>${c.closingDescription}</p></div>${cta(c.closingAction, 'rfq')}</section>
