import * as runtime from "./i18n-decision.js?v=opennext-20260914-desk-1";
...
const pairs = [
  ...
  ["使用 OpenNEXT 端点 / 密钥的报价会明确显示 Managed Gateway，不会和原厂项目访问混标。", "Quotes delivered through a OpenNEXT endpoint or key are explicitly labelled Managed Gateway and never blended with original-provider project access."],
  ...
  ["OpenNEXT 托管端点", "OpenNEXT managed endpoint"],
  ...
  ["OpenNEXT 核验内容 · 演示政策", "WHAT OPENNEXT CHECKS · DEMO POLICY"],
  ...
  ["OpenNEXT 提供 KYB、能力验证、测试执行、经纪撮合与安全交割。非原厂 / 第三方来源必须显式标注。", "OpenNEXT provides KYB, capability verification, test execution, brokered matching and secure delivery. Non-native and third-party provenance must be explicit."],
...
const zhAliases = new Map([
  ...
  ["使用 OpenNEXT endpoint / key 的报价会明确显示 Managed Gateway，不会和原厂项目访问混标。", "使用 OpenNEXT 端点 / 密钥的报价会明确显示 Managed Gateway，不会和原厂项目访问混标。"],
  ["Native、Gateway 与 Hosted 供应不混标、不混算。", "原厂、网关与托管供应不混标、不混算。"],
  ["Endpoint 健康 + 计量审计", "端点健康 + 计量审计"],
  ["OpenNEXT 托管 endpoint", "OpenNEXT 托管端点"],
  ...
  ["OpenNEXT 提供 KYB、能力验证、测试执行、经纪撮合与安全交割。Non-native / third-party 来源必须显式标注。", "OpenNEXT 提供 KYB、能力验证、测试执行、经纪撮合与安全交割。非原厂 / 第三方来源必须显式标注。"],
