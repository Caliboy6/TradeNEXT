import { state, providerDepth, escapeHtml } from "./demo-core.js?v=opennext-20260913-1";
import { rfqTypes, providerProfiles, provenanceCatalog, demandTape, quoteComparison, gpuSupplyListings, procurementState, ratingPolicy, completedTransactions, getSupplierReputation } from "./procurement-data.js?v=opennext-20260913-1";
...
function commonFields(type, preset) {
  ...
  return `<div class="form-field"><label>${amountLabel}</label><input class="input" name="amount" aria-label="RFQ budget or allocation" value="${type === "gpu" ? "$250,000" : "$100,000"}"></div><div class="form-field"><label>Term</label><select class="select" name="term" aria-label="Service term"><option value="30d">30 days</option><option value="90d">90 days</option><option value="custom">Custom term</option></select></div><div class="form-field"><label>Region</label><select class="select" name="region" aria-label="Data region"><option>US</option><option>Singapore</option><option>Europe</option><option>APAC</option></select></div><div class="form-field"><label>Required start</label><select class="select" name="start" aria-label="Required start"><option>Within 7 days</option><option>Immediately</option><option>Custom date</option></select></div>`;
}

function typeFields(type, preset = "") {
  if (type === "gpu") return `<div class="form-field form-field-wide"><label>Accelerator / cluster</label><input class="input" name="product" aria-label="Product or capacity requirement" value="${escapeHtml(preset || "H100 SXM × 64")}"></div><div class="form-field"><label>Quantity</label><input class="input" name="quantity" aria-label="Accelerator quantity" value="64 accelerators"></div><div class="form-field"><label>Topology / network</label><select class="select" name="network" aria-label="Topology or network"><option>NVLink + 400G IB</option><option>PCIe</option><option>Private network required</option></select></div>`;
  if (type === "hosted") return `<div class="form-field form-field-wide"><label>Model or managed system</label><input class="input" name="product" aria-label="Product or capacity requirement" value="${escapeHtml(preset || "Hosted inference capacity")}"></div><div class="form-field"><label>Minimum TPS</label><input class="input" name="throughput" aria-label="Minimum throughput" value="33.3K TPS"></div><div class="form-field"><label>Delivery</label><select class="select" name="delivery" aria-label="Delivery method"><option>Provider-hosted endpoint</option><option>OpenNEXT managed endpoint</option></select></div>`;
  if (type === "private_otc") return `<div class="form-field form-field-wide"><label>Capacity requirement</label><input class="input" name="product" aria-label="Product or capacity requirement" value="${escapeHtml(preset || "Claude capacity · non-standard terms")}"></div><div class="form-field"><label>Minimum TPS</label><input class="input" name="throughput" aria-label="Minimum throughput" value="33.3K TPS"></div><div class="form-field"><label>Authorization status</label><select class="select" name="authorization" aria-label="Authorization status"><option>Unknown / to be investigated</option><option>Supplier claims allocation rights</option><option>Technically verified only</option></select></div>`;
  return `<div class="form-field form-field-wide"><label>Model and version</label><input class="input" name="product" aria-label="Product or capacity requirement" value="${escapeHtml(preset || "Claude Sonnet")}"></div><div class="form-field"><label>Minimum TPS</label><input class="input" name="throughput" aria-label="Minimum throughput" value="33.3K TPS"></div><div class="form-field"><label>Accepted authorization class</label><select class="select" name="provenance" aria-label="Accepted authorization class"><option>Upstream-authorized resale only</option><option>Authorized enterprise allocation</option><option>Partner-delivered access</option></select></div>`;
}
