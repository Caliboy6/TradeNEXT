...
function publicAgentDemo() {
  ...
  const stepMarkup = steps.map(([title, summary], index) => '<div class="agent-log-step' + (index === 0 ? ' is-current' : '') + '" data-agent-step="' + index + '"><span>' + String(index + 1).padStart(2, '0') + '</span><div><strong>' + title + '</strong><p>' + summary + '</p></div><em>' + (index === 0 ? 'Live' : 'Queued') + '</em></div>').join('');
  const factSets = [
  ...
  const detailMarkup = steps.map(([title, summary, body], index) => '<section class="public-agent-demo-detail" data-agent-detail="' + index + '"' + (index ? ' hidden' : '') + '><span class="agent-kicker">Step 0' + (index + 1) + ' / automated run</span><h3>' + title + '</h3><p>' + body + '</p><div class="public-agent-demo-facts">' + factSets[index].map(([label, value]) => '<span><small>' + label + '</small><strong>' + value + '</strong></span>').join('') + '</div></section>').join('');
  const workspaceStages = [
    '<div class="public-demo-stage-kicker">REQUEST PACKAGE / DRAFT</div><h4>Minimum fields captured</h4><p>The buyer starts with a concise brief. The Agent fills only what is needed to match capacity.</p><div class="public-demo-brief-grid"><div><span>GPU</span><strong>H100</strong></div><div><span>Quantity</span><strong>64</strong></div><div><span>Region</span><strong>Singapore</strong></div><div><span>Term</span><strong>30 days</strong></div></div><div class="public-demo-stage-foot"><span>Buyer identity</span><strong>Not collected</strong><span>RFQ status</span><strong>Draft</strong></div>',
    '<div class="public-demo-stage-kicker">RFQ-2048 / GPU RENTAL</div><h4>Standard request prepared</h4><p>The request is ready for private matching with only buyer-approved specifications.</p><div class="public-demo-document"><div><span>Match constraints</span><strong>H100 · 64 GPUs · Singapore · 30 days</strong></div><div><span>Commercial frame</span><strong>$210,000 monthly budget · quote required</strong></div><div><span>Publication scope</span><strong>Qualified supply only · counterparties masked</strong></div></div><div class="public-demo-stage-foot"><span>Agent action</span><strong>Ready to screen supply</strong></div>',
    '<div class="public-demo-stage-kicker">PRIVATE SUPPLY / SCREENING</div><h4>Three supply signals compared</h4><p>Only fit signals are shown in the public presentation. Supplier identity and transaction terms stay private.</p><div class="public-demo-supply-table"><div class="public-demo-supply-head"><span>Signal</span><span>Fit</span><span>Status</span></div><div><span>Supplier 01 · masked</span><strong>82%</strong><em>Over budget</em></div><div><span>Supplier 02 · masked</span><strong>88%</strong><em>Start mismatch</em></div><div class="is-selected"><span>Supplier 03 · masked</span><strong>96%</strong><em>Best fit</em></div></div><div class="public-demo-stage-foot"><span>Verification</span><strong>Capacity · SLA · delivery checked</strong></div>',
    '<div class="public-demo-stage-kicker">SHARED DOCUMENT / V1</div><h4>GPU Rental Agreement assembled</h4><p>Buyer and seller complete one standard English document. Reusable seller terms are prefilled; shared terms remain open for confirmation.</p><div class="public-demo-agreement-grid"><div><span>Capacity</span><strong>64 × H100</strong></div><div><span>Delivery</span><strong>Singapore</strong></div><div><span>Term</span><strong>30 days</strong></div><div><span>Acceptance</span><strong>4 / 4 core terms</strong></div></div><div class="public-demo-agreement-flow"><span class="is-done">Buyer brief</span><i>→</i><span class="is-done">Seller terms</span><i>→</i><span class="is-active">Shared confirmation</span></div>',
    '<div class="public-demo-stage-kicker">APPROVAL GATE / VETA</div><h4>Best-fit quote ready for approval</h4><p>The Agent presents the selected match with masked counterparties and keeps the final action behind buyer approval.</p><div class="public-demo-approval-card"><div><span>Selected match</span><strong>Supplier 03 · masked</strong></div><div><span>Fit score</span><strong>96% · 4 / 4 terms</strong></div><div><span>Policy checks</span><strong>Passed</strong></div></div><div class="public-demo-checks"><span>✓ Request stays within buyer parameters</span><span>✓ No credentials or payment data included</span><span>✓ Final approval required before order</span></div>',
    '<div class="public-demo-stage-kicker">ORDER RECEIPT / SEALED</div><h4>Order confirmed</h4><p>The automated presentation reaches the same completion state as the workspace Agent. No payment or real allocation occurs.</p><div class="public-demo-confirmation"><span>✓</span><div><strong>ON-DEMO-2409</strong><small>64 × H100 · Singapore · 30 days</small></div><em>Capacity reserved</em></div><div class="public-demo-stage-foot"><span>Audit record</span><strong>Sealed</strong><span>Visibility</span><strong>Privacy-safe</strong></div>',
  ].map((content, index) => '<section class="public-demo-workspace-detail" data-agent-workspace-detail="' + index + '"' + (index ? ' hidden' : '') + '>' + content + '</section>').join('');
  ...
  return '<article class="public-agent-demo" data-agent-demo><header class="public-agent-demo-head"><div><p class="public-demo-eyebrow">AUTOMATED WORKSPACE PREVIEW</p><h3>OpenNEXT Agent / procurement run</h3><p>The same Agent mode used in My OpenDesk, replayed automatically as a read-only presentation. No prompt input, payment or live allocation is enabled.</p></div><div class="public-agent-demo-status"><span data-agent-demo-dot></span><strong data-agent-demo-status>Running</strong><small data-agent-demo-progress>Step 01 / 06</small></div></header><div class="public-agent-demo-shell"><aside class="public-demo-agent-panel agent-panel" aria-label="OpenNEXT Agent automated preview"><div class="agent-heading"><div class="agent-identity"><span class="agent-symbol">' + sparkle + '</span><div><h2>OpenNEXT Agent</h2><p>From request to reserved capacity</p></div></div><span class="public-demo-auto-badge">Auto run</span></div><div class="agent-context"><span>Active brief</span><strong>64 × H100 · Singapore · 30 days</strong></div><div class="agent-scroll"><div class="agent-intro"><p>Tell me what you need. I will prepare the RFQ, compare private supply and request approval before action.</p></div><div class="agent-conversation" role="log" aria-label="Automated Agent workflow" aria-live="polite"><article class="agent-message agent-message-user"><span class="agent-speaker">Buyer brief</span><p>Need 64 H100 GPUs in Singapore for 30 days, within a $210,000 monthly budget.</p></article><article class="agent-message"><span class="agent-speaker">OpenNEXT Agent</span><p data-agent-demo-message>Understood. I am capturing the minimum fields needed to start a private match.</p></article><section class="agent-card public-agent-demo-active-card"><div class="public-agent-demo-activity"><span class="agent-kicker">Execution activity · simulation</span><strong data-agent-demo-activity>Parsing the buyer brief</strong><em data-agent-demo-stage-tag>Live</em></div><div class="public-agent-demo-details">' + detailMarkup + '</div></section><section class="agent-card agent-work-log public-agent-demo-work-log"><span class="agent-kicker">Run log · no input</span>' + stepMarkup + '</section></div></div><div class="public-demo-agent-footer"><span>Approval before action</span><strong>Read-only presentation</strong></div></aside><section class="public-demo-workspace-view" aria-label="Private procurement workspace"><header class="public-demo-workspace-head"><div><p class="public-demo-eyebrow">MY OPENDESK / PRIVATE RFQ</p><h4>GPU rental workspace</h4></div><div><span class="public-demo-workspace-status" data-agent-demo-workspace-status>Simulating</span><small data-agent-demo-workspace-progress>01 / 06</small></div></header><div class="public-demo-workspace-rails"><div><span>Visibility</span><strong>Masked counterparties</strong></div><div><span>Authority</span><strong>Buyer approval required</strong></div><div><span>Record</span><strong data-agent-demo-workspace-record>Draft</strong></div></div><div class="public-demo-workspace-stage" aria-live="polite">' + workspaceStages + '</div><footer class="public-demo-workspace-footer"><span><i></i>Private workflow</span><span>Supplier identity and transaction terms remain hidden</span></footer></section></div><footer class="public-agent-demo-footer-bar"><span>Simulated order · no payment or allocation occurs</span><button type="button" data-public-demo-replay>Replay run <span aria-hidden="true">↻</span></button></footer></article>';
}
