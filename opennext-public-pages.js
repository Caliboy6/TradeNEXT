...
function gpuTicker() {
  const rows = [
    ['Supply', 'H100 GPU rental', '8 GPUs · US West · 30 days', 'Per GPU-hour · Quote based'],
    ['Supply', 'H200 GPU rental', '8 GPUs · Singapore · Flexible term', 'Per GPU-hour · Reserved capacity'],
    ['Demand', 'A100 GPU cluster', '16 GPUs · APAC · 90 days', 'Request for quote'],
  ];
  const items = rows.map(([side, resource, spec, terms]) => '<div class="public-ticker-row" data-ticker-row="' + side.toLowerCase() + '"><span class="public-ticker-side public-ticker-side-' + side.toLowerCase() + '">' + side + '</span><strong>' + resource + '</strong><span>' + spec + '</span><span class="public-ticker-terms">' + terms + '</span></div>').join('');
...
function publicAgentDemo() {
  const steps = [
    ['Parse the brief', '64 × H100 · 30 days · Singapore', 'The agent extracts only the minimum fields needed to start a private match.'],
    ['Match private supply', '3 qualified supply signals', 'Inventory is screened against model, quantity, region, term and budget.'],
    ['Verify the fit', 'SLA · topology · availability', 'Rights, delivery evidence and technical constraints are checked before outreach.'],
    ['Align rental terms', 'Shared agreement · 4/4 core terms', 'Buyer preferences and reusable seller terms are combined into one rental document.'],
    ['Request approval', 'Buyer review · no identity exposure', 'The selected quote is presented with masked counterparties and a complete audit trail.'],
  ...
  const stepMarkup = steps.map(([title, copy], index) => '<li data-agent-step="' + index + '"' + (index === 0 ? ' class="is-active"' : '') + '><span>0' + (index + 1) + '</span><div><strong>' + title + '</strong><small>' + copy + '</small></div><em>' + (index === 0 ? 'Live' : 'Queued') + '</em></li>').join('');
  const detailMarkup = steps.map(([title, copy, body], index) => '<div class="public-agent-demo-detail" data-agent-detail="' + index + '"' + (index ? ' hidden' : '') + '><p class="public-demo-eyebrow">STEP 0' + (index + 1) + ' / ' + title.toUpperCase() + '</p><h4>' + copy + '</h4><p>' + body + '</p><div class="public-agent-demo-facts"><span><small>Counterparty</small><strong>Masked</strong></span><span><small>Policy check</small><strong>' + (index >= 2 ? 'Passed' : 'Queued') + '</strong></span><span><small>Record</small><strong>' + (index === 5 ? 'Sealed' : 'Draft') + '</strong></span></div></div>').join('');
  return '<article class="public-agent-demo" data-agent-demo><header class="public-agent-demo-head"><div><p class="public-demo-eyebrow">AGENT RUN / PRESENTATION MODE</p><h3>From private RFQ to confirmed order.</h3><p>Watch the workspace agent assemble a match, align the rental terms and place a simulated order. This preview has no input controls.</p></div><div class="public-agent-demo-status"><span data-agent-demo-dot></span><strong data-agent-demo-status>Running</strong><small data-agent-demo-progress>Step 01 / 06</small></div></header><div class="public-agent-demo-grid"><ol class="public-agent-demo-steps" aria-label="Agent demo steps">' + stepMarkup + '</ol><section class="public-agent-demo-console" aria-live="polite"><div class="public-agent-demo-console-bar"><span>OpenNEXT Agent</span><span>Private workflow</span></div><div class="public-agent-demo-details">' + detailMarkup + '</div><div class="public-agent-demo-order"><span class="public-demo-eyebrow">ORDER STATE</span><strong data-agent-demo-order-status>Preparing match</strong><small>Buyer and seller identities remain private.</small></div></section></div><footer><span>Simulated order · no payment or allocation occurs</span><button type="button" data-public-demo-replay>Replay demo <span aria-hidden="true">↻</span></button></footer></article>';
}
...
export function renderLanding(locale = 'en') {
  ...
  return `<div class="public-page" lang="${locale}">
    ...
    <main>
      ...
      <section class="public-listings-band" id="inside-opennext" aria-labelledby="public-listings-title"><div class="public-section public-listings-section public-container"><div class="public-section-heading"><p class="on-eyebrow">${c.inside}</p><h2 id="public-listings-title">${c.listingsTitle}</h2><p class="public-section-description">${c.listingsDescription}</p></div><div class="public-preview-label">${c.example}</div>${listings(c)}<ol class="public-posting-steps">${c.steps.map((step, i) => `<li><span>0${i + 1}</span><p>${step}</p></li>`).join('')}</ol><div class="public-preview-actions">${cta(c.viewMarket)}${cta(c.publish, 'supply', 'text')}</div></div></section>
      <section class="public-section public-market-section public-container" id="market" aria-labelledby="public-market-title"><div class="public-section-heading"><p class="on-eyebrow">${c.marketLabel}</p><h2 id="public-market-title">${c.marketTitle}</h2></div><div class="public-market-row"><span class="public-row-number">01</span><h3>${c.models}</h3><p>${c.modelDescription}</p><span class="public-market-status">${c.tokenStatus}</span></div><div class="public-market-row"><span class="public-row-number">02</span><h3>${c.gpu}</h3><p>${c.gpuDescription}</p><span class="public-market-status">${c.gpuStatus}</span></div></section>
