...
import { escapeHtml } from "./demo-core.js?v=opennext-20260914-desk-1";
import {
  demandTape,
  quoteComparison,
  gpuSupplyListings,
...
function capacityForm(type) {
  if (type === "accelerator") return `<form class="flow-form"><label><span>Accelerator</span><select class="select"><option>H100 SXM</option><option>H200</option><option>B200</option><option>L40S</option></select></label><label><span>Available units</span><input class="input" type="number" value="32"></label><label><span>Topology</span><input class="input" value="8-GPU node · NVLink"></label><label><span>Tenancy</span><select class="select"><option>Dedicated</option><option>Shared</option></select></label><label><span>Location</span><select class="select"><option>Singapore</option><option>US East</option><option>Frankfurt</option></select></label><label><span>Available from</span><input class="input" type="datetime-local" lang="en-US" value="${localDateInput(nextReservationStart())}"></label><label><span>Available term</span><input class="input" value="30–90 days"></label><label><span>Indicative hourly rate</span><input class="input" value="$2.34 / accelerator·h"></label><label class="span-2"><span>Network, storage and calendar notes</span><textarea class="textarea" rows="3">400G InfiniBand, 2TB NVMe per node, protected calendar blocks disclosed before reservation.</textarea></label></form>`;
...
function buyerStepBody() {
  ...
  if (step === 1) return `${progress(["Quote", "Lock", "Test", "Contract", "Activate", "Receipt"], 1)}<form class="flow-form"><label><span>Allocation to lock</span><input class="input" value="${quote.capacity}"></label><label><span>Commercial total</span><input class="input" value="${quote.total}"></label><label><span>Service start</span><input class="input" type="datetime-local" lang="en-US" value="${localDateInput(nextReservationStart())}"></label><label><span>Service term</span><input class="input" value="30 days"></label><label class="span-2 check-row"><input type="checkbox" id="commercialConfirmed" checked> I accept the quote validity, delivery, revocation and refund boundaries.</label></form><div class="workflow-callout"><span>✓</span><div><strong>Allocation hold available</strong><p>The supplier will hold the quoted capacity for 45 minutes while acceptance testing is completed.</p></div></div>`;
  ...
  if (step === 3) return `${progress(["Quote", "Lock", "Test", "Contract", "Activate", "Receipt"], 3)}<form class="flow-form"><label><span>Contract entity</span><input class="input" value="Example Organization"></label><label><span>Settlement method</span><select class="select"><option>Escrow · USD wire</option><option>Workspace balance</option></select></label><label><span>Billing contact</span><input class="input" value="billing@example.com"></label><label><span>Purchase order</span><input class="input" value="PO-2026-0819"></label><label class="span-2 check-row"><input type="checkbox" id="contractConfirmed" checked> Contract, data processing terms and settlement instruction reviewed.</label></form>`;
  if (step === 4) return `${progress(["Quote", "Lock", "Test", "Contract", "Activate", "Receipt"], 4)}<div class="activation-panel"><div><span>1</span><strong>Allocation created</strong><small>Project ON-CAP-1938</small></div><div><span>2</span><strong>Buyer access assigned</strong><small>Two workspace administrators</small></div><div><span>3</span><strong>Metering connected</strong><small>Token usage and TPS telemetry active</small></div><div><span>4</span><strong>Support channel opened</strong><small>Supplier response SLA enabled</small></div></div><div class="workflow-callout is-neutral"><span>i</span><div><strong>Sample token bundle</strong><p>This demo activation adds 10M input and 2M output tokens to My Tokens, priced at the selected supplier's token rates. It is a separate sample bundle, not a conversion of the RFQ's OEV budget.</p></div></div><label class="check-row"><input id="activationConfirmed" type="checkbox" checked> I confirm that access and metering are working as agreed.</label>`;
  return `${progress(["Quote", "Lock", "Test", "Contract", "Activate", "Receipt"], 5)}<div class="workflow-success"><span>✓</span><h3>Capacity activated</h3><p>The sample token bundle is available in My Tokens with separate input/output rates, usage tracking and a 30-day term.</p><div><span>Order</span><strong>${escapeHtml(activatedQuotes.get(`${buyerFlow.rfqId}:${buyerFlow.supplier}`) || "Sample allocation")}</strong></div><div><span>Sample token allocation</span><strong>10M input + 2M output tokens</strong></div><div><span>Service term</span><strong>30 days from activation</strong></div><div><span>Review eligibility</span><strong>After service term ends</strong></div></div>`;
}
...
function showBuyerFlow(rfqId, supplier = quoteComparison[0].supplier, step = 0) {
  ...
  const next = step < 5 ? `<button class="primary-button compact" type="button" data-flow-action="buyer-next">${labels[step]}</button>` : `<button class="primary-button compact" type="button" data-route="tokens">View My Tokens</button>`;
...
function confirmGpuReservation() {
  ...
  const body = `<div class="workflow-success"><span>✓</span><h3>GPU reservation confirmed</h3><p>Your reserved accelerator-hours are available in My GPUs. The service starts at the time shown below.</p><div><span>Reservation ID</span><strong>${escapeHtml(gpuFlow.confirmedId)}</strong></div><div><span>Start · local time</span><strong>${calculation.fmt(calculation.start)}</strong></div><div><span>End · local time</span><strong>${calculation.fmt(calculation.end)}</strong></div><div><span>Quantity</span><strong>${calculation.quantity} × ${escapeHtml(item.accelerator)}</strong></div><div><span>Total</span><strong>$${calculation.total.toLocaleString("en-US", { maximumFractionDigits: 2 })}</strong></div><div><span>Extension availability</span><strong>${calculation.extension ? `Up to ${calculation.extension} hours · not purchased` : "Not requested"}</strong></div></div>`;
  const footer = `<button class="secondary-button compact" type="button" data-flow-action="open-chat" data-supplier="${escapeHtml(item.supplier)}" data-context="${escapeHtml(gpuFlow.confirmedId)}">Message operator</button><button class="primary-button compact" type="button" data-route="my-gpus">View My GPUs</button>`;
...
function handleAction(action, element) {
  ...
  if (action === "buyer-next") {
    if (buyerFlow.step === 1 && !modal.querySelector("#commercialConfirmed")?.checked) return showToast("Review the terms", "Accept the sample commercial terms before continuing");
    if (buyerFlow.step === 3 && !modal.querySelector("#contractConfirmed")?.checked) return showToast("Review the agreement", "Confirm the sample agreement review before continuing");
    if (buyerFlow.step === 4) {
      ...
      if (!activatedQuotes.has(key)) {
        ...
        const allocation = window.OpenNEXTAddAllocation?.({ kind: "tokens", sourceKey: key, model: item.model, quantity, inputTokens, outputTokens, inputRate: quote.inputPer1MUsd, outputRate: quote.outputPer1MUsd, rate, supplier: buyerFlow.supplier, region: item.region, days: 30 });
