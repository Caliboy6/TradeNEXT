...
export function renderOverview() {
  ...
  return `<div class="page phase1-overview">
    ...
    <section class="section"><div class="section-header"><div><div class="eyebrow">PROVENANCE-FIRST MARKET</div><h2>先看来源，再比较价格</h2><p>TradeNEXT 不把所有供应抽象成同一种 API。买方在报价之前就知道容量从哪里来、最终拿到什么。</p></div><button class="text-link" type="button" data-route="models">查看全部 Model Capacity →</button></div><div class="provenance-market-grid">${nativeModels.map((model) => modelProvenanceCard(model, "native")).join("")}</div></section>
...
export function renderModels() {
  ...
  return `<div class="page phase1-models">
    ...
    <div class="provenance-explainer"><div><span class="shield-icon">✓</span><div><strong>Native 不是裸 Key</strong><p>TradeNEXT 验证来源证据与容量测试；买方获得清晰约定的项目访问、专属配额或合作方交付，不公开秘密凭证。</p></div></div><button class="text-link" type="button" data-phase-action="view-verification">查看验证标准 →</button></div>
    ...
    <section class="section model-policy-grid"><div><span class="badge badge-green">CORE</span><h3>Native Direct / Allocated</h3><p>价格发现、容量发现、RFQ、验证、分配和结算，是 TradeNEXT 的 Phase 1 主市场。</p></div><div><span class="badge badge-amber">SEPARATE LANE</span><h3>Private OTC</h3><p>大额或复杂供应进入私密撮合，不公开库存，不进入 Native 基准指数。</p><button class="text-link" type="button" data-phase-action="private-otc">进入 OTC Desk →</button></div><div><span class="badge badge-gray">OPTIONAL</span><h3>Managed Gateway</h3><p>只有买方主动选择时才启用；不与 Native 供应混标，也不是平台核心 wedge。</p></div></section>
  </div>`;
}
